...
static enum depgraph {
        DEP_ALL,
        DEP_ORDER,
        DEP_REQUIRE
} arg_depgraph = DEP_ALL;
...
static char *pretty_boot_time(DBusConnection *bus)
{
        ...
        if (t->firmware_time)
                size = strpcpyf(&ptr, size, "%llums (firmware) + ", t->firmware_time - t->loader_time);
        if (t->loader_time)
                size = strpcpyf(&ptr, size, "%llums (loader) + ", t->loader_time);
        if (t->kernel_time)
                size = strpcpyf(&ptr, size, "%llums (kernel) + ", t->kernel_done_time);
        if (t->initrd_time > 0)
                size = strpcpyf(&ptr, size, "%llums (initrd) + ", t->userspace_time - t->initrd_time);

        size = strpcpyf(&ptr, size, "%llums (userspace) ", t->finish_time - t->userspace_time);
        if (t->kernel_time > 0)
                size = strpcpyf(&ptr, size, "= %llums", t->firmware_time + t->finish_time);
        else
                size = strpcpyf(&ptr, size, "= %llums", t->finish_time - t->userspace_time);
...
static int analyze_plot(DBusConnection *bus)
{
        ...
        for (struct unit_times *u = times; u < times + n; u++) {
                ...
                svg_text(u->ixt, y, u->time? "%s (%llums)" : "%s", u->name, u->time);
                y++;
        }
        svg("</g>\n\n");
...
static int graph_one_property(const char *name, const char *prop, DBusMessageIter *iter) {
        ...
        if (arg_depgraph != DEP_ALL)
                if ((arg_depgraph == DEP_ORDER) != streq(prop, "After"))
                        return 0;
...
static int depgraph(DBusConnection *bus) {
        _cleanup_dbus_message_unref_ DBusMessage *reply = NULL;
        DBusMessageIter iter, sub;
        int r;
        ...
        while (true) {
                switch (getopt_long(argc, argv, "h", options, NULL)) {
                        ...
                        case ARG_SYSTEM:
                                ...
                                break;
                        case ARG_ORDER:
                                arg_depgraph = DEP_ORDER;
                                break;
                        case ARG_REQUIRE:
                                arg_depgraph = DEP_REQUIRE;
                                break;
                        case -1:
                                return 1;
                        case '?':
...
int main(int argc, char *argv[]) {
        ...
        if (!argv[optind] || streq(argv[optind], "time"))
                ...
                r = analyze_plot(bus);
        else if (streq(argv[optind], "depgraph"))
                r = depgraph(bus);
        else
